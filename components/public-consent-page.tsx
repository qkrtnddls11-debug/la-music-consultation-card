...
export function PublicConsentPage({ token }: { token: string }) {
  ...
  return <RegistrationConsentFlow consultation={consultation} branchName={(consultation as { branch_name?: string }).branch_name} submitUrl={`/api/sign/${encodeURIComponent(token)}`} publicLink onClose={() => window.close()} onComplete={() => setComplete(true)} />;
}
