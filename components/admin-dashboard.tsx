              ...
              return <article key={reservation.id} className={`rounded-[18px] p-4 shadow-[0_2px_10px_rgba(0,0,0,0.05)] sm:p-5 ${todayConfirmed ? "border-2 border-[#e8a23d] bg-amber-50" : "bg-white"}`}>
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="min-w-[230px] flex-1">
                    ...
                    <div className="mt-3 rounded-xl bg-white/80 p-3 text-sm leading-6 text-[#5f584e]">{reservation.schedule_preferences.map((item) => <p key={item.rank}><strong>{item.rank}순위</strong> · {item.day} {item.timeText}</p>)}</div>
